...
function addClassificationTypeQuestion() {
	...
	var str = '<div class="col-xs-12">'
			...
			+ '<label class="requireFld" id="classificationTypeQuestionAnswerExplanationError" >This Field is required.</label>'
			+ '</div>'
            
			+ '<div class="col-xs-12 form-group"></div>'
			+ '<div class="col-xs-12 form-group questionMark" id="classificationTypeQuestionMarkDiv">'
			+ '<label class="form-group"><sup><font color="red" size="3px">&nbsp;*</font></sup><span>'+messages['lbl.enterthemarkforthisquestion']+'</label>'
			+ '<input type="text" style="width: 50px" name="classificationTypeQuestionMark" class="form-control" id="classificationTypeQuestionMark" onkeyup="classificationTypeQuestionListner();" maxlength="2">'
			+ '<label class="requireFld" id="classificationTypeQuestionMarkError1">'+messages['msg.empty']+'</label><label class="requireFld" id="classificationTypeQuestionMarkError2">'+messages['msg.allowednumericvalue']+'</label>'
			+ '</div>'
	...
	if (equalMarkTest == 1) {
		$("#classificationTypeQuestionMark").val(everyQuestionMark);
		$("#classificationTypeQuestionMarkDiv").hide();
	}
...
function classificationTypeQuestionValidate() {
	...
	if (equalMarkTest == 0) {
		if ($("#classificationTypeQuestionMark").val() == "") {
			$("#classificationTypeQuestionMark").css("border-color", "#c95b5b");
			$("#classificationTypeQuestionMarkError1").fadeIn();
			return false;
		} else if (!($("#classificationTypeQuestionMark").val()).match(/^\d*$/)) {
			$("#classificationTypeQuestionMark").css("border-color", "#c95b5b");
			$("#classificationTypeQuestionMarkError2").fadeIn();
			return false;
		}
	}
	/*
...
function classificationTypeQuestionListner() {
	...
	if ($("#classificationTypeQuestionMark").val().length > 0) {
		$("#classificationTypeQuestionMark").css("border-color", "");
		$("#classificationTypeQuestionMarkError1").fadeOut();
		$("#classificationTypeQuestionMarkError2").fadeOut();
	}
...
function clearQuestionDataInClassificationTypeQuestion() {
	...
	if (equalMarkTest != 1) {
		$("#classificationTypeQuestionMark").val('');
	}
...
function saveClassificationTypeQuestionInJson() {
	...
	if (classificationTypeQuestionValidate()) {
		...
		var quesData = createClassificationTypeQuestionJson(position);
 ...
 * @returns {Array}
 */
function createClassificationTypeQuestionJson(position) {
	var quesData = [];
	var optionsList = [];
	var questionContent = CKEDITOR.instances['classificationTypeQuestion']
			.getData();
	var question = {
		...
		"questionMark" : parseInt($("#classificationTypeQuestionMark").val()),
		"negMark" : (parseInt($("#classificationTypeQuestionMark").val()) * negMark) / 100,
 ...
 * @returns no.
 */
function fillClassificationTypeQuestionData(position, questionId) {
	for (var i = 0; i < questionSectionList[position].questionList.length; i++) {
		if (questionSectionList[position].questionList[i].questionId == questionId) {
			...
			var updateAction = 'updateClassificationQuestionTypeJson("'
					+ position + '","' + questionId + '")';
 ...
 * @returns no.
 */
function updateClassificationQuestionTypeJson(position, questionId) {
	if (classificationTypeQuestionValidate()) {
		for (var q = 0; q < questionSectionList[position].questionList.length; q++) {
			if (questionSectionList[position].questionList[q].questionId == questionId) {
				...
				var question = {
					...
					"questionName" : questionContent,
					"questionNo" : (q + 1),
					"questionMark" : parseInt($(
							"#classificationTypeQuestionMark").val()),
					"negMark" : (parseInt($("#classificationTypeQuestionMark")
							.val()) * negMark) / 100,
