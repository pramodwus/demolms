...
function sortListTypeQuestionPreview(quesData) {
	...
	try {
		$("#allquestion").append(createSortListTypeQuestionList());
  		...
  		var answerValue=quesData.answerValue.indexOf(',')!=-1?quesData.answerValue.split(','):quesData.answerValue;
  		for(var optn=0;optn<quesData.option.length;optn++){
  			var optionid=quesData.option[optn].optionId;
  			$("#usersourceoptionlist").append(addSortListQuestionOptions(optn+1));
  			$("#usertargetoptionlist").append(addSortListTypeTargetOptions(optn+1));
  			$("#sourceOptionName"+(optn+1)).html(quesData.option[optn].optionName);
  			$("#targetOptionName"+(optn+1)).html(quesData.option[answerValue[optn]-1].optionName);
  		}
  		quesData.explanation==null ? $("#description").hide():(quesData.explanation.trim().length==0?$("#description").hide():$("#ansexplain").html(quesData.explanation));
...
function createSortListTypeQuestionList() {
	var questnString = '<div class="box-body box-left-margin questionAllDiv" id="questionDiv">'
			...
			+ '<h3>Explanation</h3><p id="ansexplain"></p></div>'
			+ '</div>';
...
function addSortListQuestionOptions(optn) {
	var opnHTML = '<tr><td class="source_drag"><div class="col-sm-12 lrn_btn_sort" id="sourceOptionOrder'
			...
			+ optn
			+ '</div>&nbsp;'
			+ '<div class="imgset pull-left padding-left" id="sourceOptionName'+optn+'">'
			+ '</div>' + '</div>' + '</div></td></tr>';
	return opnHTML;
}
...
function addSortListTypeTargetOptions(optn) {
	var opnHTML = '<tr><td class="target_drag"><div class="col-sm-12 lrn_btn_sort" id="targetOptionOrder'
		...
		+ optn
		+ '</div>&nbsp;'
		+ '<div class="imgset pull-left padding-left" id="targetOptionName'+optn+'">'
		+ '</div>' + '</div>' + '</div></td></tr>';
	return opnHTML;
}