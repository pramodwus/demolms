...
function addMatchListTypeQuestion() {
	...
	var str = '<div class="col-xs-12">'
			...
			+ '<label class="row form-group">'+messages['lbl.matchinglistorder']+' &nbsp;<span class="question-badge bg-gray questiontype-tooltip" id="question-matchlist-order-tooltip-title"><i class="fa fa-question"></i></span></label>'
			+ '<div class="row form-group" style="border:1px solid #ccc;padding:15px;padding-bottom:0px">'
...
function saveMatchListTypeQuestionInJson() {
	if (matchListTypeQuestionValidate()) {
		var quesData = createMatchListTypeQuestionJson();
 ...
 * @returns {Array}
 */
function createMatchListTypeQuestionJson() {
	var quesData = [];
	var optionsList = [];
	var questionContent = CKEDITOR.instances['matchListTypeQuestion'].getData();
	var question = {
 ...
 * @returns no.
 */
function fillMatchListTypeQuestionData(questionId) {
 ...
 * @returns no
 */
function updateMatchListTypeQuestionPopUp(questionId) {
	if (matchListTypeQuestionValidate()) {
		$("#clearquestionAlert p").text(messages['msg.sureforquestionupdate']);
		$("#dId")
				.attr('onclick', 'updateMatchListQuestion(' + questionId + ')');
		$("#clearquestionAlert").modal('show');
	}
}

/**
 * @summary This is used for performing opertion for update the match list
 *          type question.
 * @param questionId
 * @retruns no
 */
function updateMatchListQuestion(questionId) {
	if (matchListTypeQuestionValidate()) {
		var quesData = updateMatchListQuestionTypeJson(questionId);
		submitQuestionDetails();
	}
}
/**
 ...
 * @returns no.
 */
function updateMatchListQuestionTypeJson(questionId) {
	if (matchListTypeQuestionValidate()) {
