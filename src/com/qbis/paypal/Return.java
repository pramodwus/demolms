...
public class Return extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("finalPayment Amount Is "+finalPaymentAmount);
    		...
    		*/
    		...
    		if(strAck !=null && (strAck.equalsIgnoreCase("SUCCESS") || strAck.equalsIgnoreCase("SUCCESSWITHWARNING") ))
    		{
    			...
    			*/
    			...
    			/*System.out.println("Total Amount "+totalAmt+"  payerId+++++++++++++++"+getec.get("L_PAYMENTREQUEST_0_NUMBER0"));*/
    	...
    	*/
    	...
    	if (isSet(request.getParameter("page")) && request.getParameter("page").equals("return")){  
	    	...
	    	if(strAck !=null && (strAck.equalsIgnoreCase("Success") || strAck.equalsIgnoreCase("SuccessWithWarning"))){
	    		...
	    		System.out.println("else Block++++++++++++++++++++))))))0000000000000000000000000000");
		    	//Display a user friendly Error on the page using any of the following error information returned by PayPal
	    	...
	    	if(session.getAttribute("action").toString().equalsIgnoreCase("signup"))
    		{
	    		 page="pages/returns.jsp";
    		}
    		else
    		{
    			 page="pages/return.jsp";
    		}
	       
	        User user=(User) request.getSession().getAttribute("userSession");
	        QbisLicenseService license=new QbisLicenseService();
  		    license.updateLicenseIdInOrg(user.getOrgId(),Integer.parseInt((String)(session.getAttribute("licenseId"))));
  		    license.saveTransectionDetails(user.getOrgId(), Integer.parseInt((String)(session.getAttribute("licenseId"))), result.get("PAYMENTINFO_0_PAYMENTSTATUS"), result.get("PAYMENTINFO_0_TRANSACTIONID"),result.get("PAYMENTINFO_0_AMT"));
    	}else{
    		 User user=(User) request.getSession().getAttribute("userSession");	 
    		 System.out.println("Action is ))))))))))))"+session.getAttribute("action"));
    		if(session.getAttribute("action").toString().equalsIgnoreCase("signup"))
    		{
    			page="pages/reviews.jsp";
    		}
    		else
    		{
    			page="pages/review.jsp";
    		}
